...
def test_base_import_yolo():
    ...
    out_json_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),'tmp','lsc-pytest','yolo_exp_test.json')
    ...
    out_config_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),'tmp','lsc-pytest','yolo_exp_test.label_config.xml')
...
def test_base_import_yolo_with_prediction_scores():
    """Tests generated config and json files for yolo imports with confidence scores as "predictions"
    test_import_yolo_data folder assumes only images in the 'images' folder
    with corresponding labels and prediction scores existing in the 'labels' dir and a 'classes.txt' present.
    (currently 7 images -> 3 png, 2 jpg and 2 jpeg files)
    """
    input_data_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)),'data','test_import_yolo_predict_data')
    out_json_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),'tmp','lsc-pytest','yolo_exp_test.json')
    os.makedirs(Path(out_json_file).parent, exist_ok=True)
    image_ext = '.jpg,.jpeg,.png' #comma seperated string of extns.
...
if __name__ == '__main__':
    ...
    test_base_import_yolo_with_prediction_scores()